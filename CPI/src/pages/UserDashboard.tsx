...
const UserDashboard = () => {
  ...
  const [stats, setStats] = useState<any>({ totalOffers: 0, placementRate: 0, avgPackage: 0, totalCompanies: 0 });

  useEffect(() => {
    const load = async () => {
      try {
        if (!profile?.collegeId) return;
        const s = await placementApi.getStats(profile.collegeId);
        // Transform to fit existing UI expectations
        setBranchData(s.branchWise.map((b: any) => ({ branch: b.branch, placementRate: b.count })));
        setCollegeData(s.companyWise.map((c: any) => ({ name: c.company, placementRate: c.count })));
        setTrendData(s.yearWise.map((y: any) => ({ year: y.year, placementRate: y.count, avgPackage: y.avgPackage })));
        setStats({
          totalOffers: s.branchWise.reduce((a: number, b: any) => a + b.count, 0),
          placementRate: 0,
          avgPackage: Number(s.packageStats.avg?.toFixed?.(2) ?? s.packageStats.avg),
          totalCompanies: s.companyWise.length,
        });
      } catch (e) {
